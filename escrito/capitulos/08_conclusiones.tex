...
\begin{enumerate}
    ...
    \item \textbf{Modelos Predictivos de Clasificación (Random Forest):} La regularidad del
          cuarto semestre (\texttt{s4\_reg}) fue el predictor Gini más importante en el Modelo
          de Alerta Temprana, y la del octavo (\texttt{s8\_reg}) en el Modelo de Trayectoria
          Completa. Ningún predictor de promedio individual figuró entre los diez más influyentes.
...
\subsubsection{Enfoque Multiclase — Random Forest}

El sistema de alerta temprana (Modelo~A, S1--S4) clasificó correctamente el estado académico
final del $65.5\,\%$ de los estudiantes (IC~95\,\%: $[64.1, 66.9]$\,\%). El modelo de
trayectoria completa (Modelo~B, S1--S8) elevó este porcentaje al $78.2\,\%$
(IC~95\,\%: $[77.0, 79.4]$\,\%). La prueba de McNemar confirmó que esta diferencia de
$+12.7$ puntos es estadísticamente significativa ($\chi^2 = 827.70$,
$p = 5.13 \times 10^{-182}$), no un artefacto del muestreo.
